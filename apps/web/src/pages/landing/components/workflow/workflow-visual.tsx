...
import { StageContent } from '@/pages/landing/components/workflow/stage-content';
import { WORKFLOW_STEPS } from '@/pages/landing/constants/landing-content';
import {
    getSafeWorkflowIndex,
    getWorkflowProgress,
    getWorkflowStepState,
} from '@/pages/landing/helpers/workflow';
...
export function WorkflowVisual({ activeIndex, compact = false }: WorkflowVisualProps) {
    ...
    return (
        <div
            className={`border-border/70 bg-card shadow-foreground/6 relative overflow-hidden rounded-[1.75rem] border shadow-xl ${
                compact ? 'mt-8' : 'w-full'
            }`}
        >
